...
2. Find below some advanced options that can be set in the Azure ARM template file `azuredeploy.json`:
  ...
  - `chef_license_bypass`: Set to `"true"` to explicitly opt into the deprecated, unlicensed download path via `omnitruck.chef.io` when no `chef_license_key` is provided. Without this, installation fails fast with an error if `chef_license_key` is missing.
