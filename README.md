...
2. Find below some advanced options that can be set in the Azure ARM template file `azuredeploy.json`:
  ...
  - `chef_license_bypass`: Set to `"true"` to explicitly opt into the deprecated, unlicensed omnitruck download path when no `chef_license_key` is provided. Without this (or `chef_download_community`), installation fails fast with an error if `chef_license_key` is missing.
  - `chef_download_community`: Set to `"true"` to download Chef Infra Client from `chefdownload-community.chef.io` instead of the default `chefdownload-commercial.chef.io`. The community endpoint serves free/open-source builds and does not require a `chef_license_key`.
