...
# Warn when requesting an older Chef version that requires a commercial license to download
if [[ -n "${CHEF_INFRA_VERSION}" ]]; then
  _major_ver="${CHEF_INFRA_VERSION%%.*}"
  if [[ "${_major_ver}" -lt 18 ]] 2>/dev/null; then
    warn "CHEF_INFRA_VERSION=${CHEF_INFRA_VERSION}: Chef Infra Client versions < 18 require a valid license key to download from packages.chef.io."
    warn "Ensure LICENSE_KEY is set (free trial keys are accepted) — the extension will append ?licenseId=<key> to the download URL."
  fi
fi
...
echo \"  patched: ${rel_path}\""
  ...
  az vm run-command invoke \
    ...
    --name "${LINUX_VM}" \
    --command-id RunShellScript \
    --scripts "dpkg --purge chef 2>/dev/null || true; echo 'Chef purge done'" \
    --query "value[0].message" -o tsv
